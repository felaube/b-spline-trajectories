...
while true
    ...
    u_0_dot = u_dot(num_trav + 1);
    v_0_dot = v_dot(num_trav + 1);
    w_0_dot = w_dot(num_trav + 1);

    u_0_dot_dot = u_dot_dot(num_trav + 1);
    v_0_dot_dot = v_dot_dot(num_trav + 1);
    w_0_dot_dot = w_dot_dot(num_trav + 1);

    x_0 = x(num_trav + 1);
    y_0 = y(num_trav + 1);
    z_0 = z(num_trav + 1);
    
    phi_current = phi(num_trav + 1);
    T_current = T(num_trav + 1);
    n_current = load_factor(num_trav + 1);
    
    % Save initial points in past trajectory variables
    past_V = [past_V V(1:num_trav)];
    past_x = [past_x x(1:num_trav)];
    past_y = [past_y y(1:num_trav)];
    past_z = [past_z z(1:num_trav)];
    past_psi = [past_psi psi(1:num_trav)];
    past_gamma = [past_gamma gamma(1:num_trav)];
    past_u = [past_u u(1:num_trav)];
    past_v = [past_v v(1:num_trav)];
    past_w = [past_w w(1:num_trav)];
    ...
    x_obs = x_obs + u_obs*(t_diff(1)*num_trav);
    y_obs = y_obs + v_obs*(t_diff(1)*num_trav);
    z_obs = z_obs + w_obs*(t_diff(1)*num_trav);
